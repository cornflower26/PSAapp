...
int main() {
    ...
    for (int k = 0; k < 256; k++) {
        ChunkReader reader("../files/CoeffOutput.txt", 1024);
        std::vector<ChunkReader> variables;
        variables.reserve(128);

        for (int a = 0; a < 128; a++) {
            std::string name = "../files/x" + std::to_string(a);
            name += "variable.txt";
            variables.push_back(ChunkReader(name, 1024));
        }
        ...
        while (reader.hasNext()) {
            ...
            if (flag) std::cout << "there's something very wrong here" << std::endl;
